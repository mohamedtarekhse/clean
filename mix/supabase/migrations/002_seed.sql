-- ═══════════════════════════════════════════════════════════════════════════════
--  Asset Management System — Seed Data
--  Mirrors the exact data from the HTML application
-- ═══════════════════════════════════════════════════════════════════════════════
...
INSERT INTO bom_items (id,asset_id,parent_id,name,part_no,type,serial,manufacturer,qty,uom,unit_cost,lead_time,status,notes) VALUES
  ...
  ('BOM-016','AST-003',NULL,    'Mud Pump Assembly (3x)',   'MP-1103',   'Serialized','MP-1103-SET','National',      3,'EA',  620000, 180,'Active','National 14-P-220 triplex mud pumps'),
  ('BOM-017','AST-003','BOM-016','Liner Assembly 7"',       'LNR-7IN',   'Serialized',NULL,         'National',      6,'EA',  4200,   21, 'Active','7" ceramic liner, wear-resistant'),
  ('BOM-018','AST-003','BOM-016','Piston / Rod Assembly',   'PST-R201',  'Serialized',NULL,         'National',      9,'EA',  1800,   14, 'Maintenance','3 per pump, chrome-plated rods'),
  ('BOM-019','AST-003','BOM-016','Valve & Seat Kit',        'VLV-SK31',  'Bulk',       NULL,        'National',     12,'SET',650,    7,  'Active','Suction & discharge valve kits'),
  ('BOM-020','AST-003','BOM-016','Packing Seal Kit',        'PCK-S220',  'Bulk',       NULL,        'Garlock',      18,'KIT',280,    5,  'Active','Piston packing sets per pump'),
  ('BOM-021','AST-005',NULL,    'CAT 3516 Generator Set',   'GEN-7712',  'Serialized','GEN-7712-A','Caterpillar',   1,'EA',  480000, 240,'Active','2000kW prime power genset'),
  ('BOM-022','AST-005','BOM-021','CAT 3516 Engine Block',   'ENG-3516B', 'Serialized','3516B-9921','Caterpillar',   1,'EA',  210000, 120,'Active','V16 diesel engine block assembly'),
  ('BOM-023','AST-005','BOM-021','Alternator SR4B',         'ALT-SR4B',  'Serialized','SR4B-5521', 'Caterpillar',   1,'EA',  95000,  90, 'Active','2250 kVA SR4B brushless alternator'),
  ('BOM-024','AST-005','BOM-021','Engine Oil 15W-40',       'OIL-1540',  'Bulk',       NULL,        'Shell Rimula', 300,'L', 4.5,   2,  'Active','CAT DEOTF spec diesel engine oil'),
  ('BOM-025','AST-005','BOM-021','Fuel Filter Set',         'FLT-F401',  'Bulk',       NULL,        'Caterpillar',   6,'EA',  85,     3,  'Active','Primary & secondary fuel filters'),
  ('BOM-026','AST-005','BOM-021','Air Filter Assembly',     'FLT-A201',  'Bulk',       NULL,        'Donaldson',     4,'EA',  120,    5,  'Active','Heavy-duty radial seal air filters')
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO maintenance_schedules (id,asset_id,task,type,freq,last_done,next_due,tech,hours,cost,priority,status,alert_days,notes) VALUES
  ...
  ('PM-002','AST-001','Gearbox Oil Change',             'Oil Change',         90,'2024-11-01','2025-01-30','Mohammed Al-Farsi',    4,1200, 'High',    'Scheduled',14,'Drain and replace ISO 320 gear oil. Check for metal particles.'),
  ('PM-003','AST-002','BOP Pressure Test',              'Pressure Test',     180,'2024-07-10','2025-01-10','Bureau Veritas',        8,4500, 'Critical','Scheduled',30,'15K psi pressure test per API 16A. Witnessed by third party inspector.'),
  ('PM-004','AST-002','Ram Packer Inspection',          'Inspection',         90,'2024-10-15','2025-01-13','Omar Hassan',           6,2200, 'High',    'Scheduled',14,'Inspect and replace worn ram packers. Check sealing surfaces.'),
  ('PM-005','AST-003','Mud Pump Liner Inspection',      'Inspection',         30,'2024-12-20','2025-01-19','Ahmed Khalid',          3,800,  'High',    'Scheduled', 7,'Inspect liner wear. Replace if >15% worn. Check piston seals.'),
  ('PM-006','AST-003','Valve & Seat Replacement',       'Filter Replacement', 60,'2024-11-25','2025-01-24','Ahmed Khalid',          5,3200, 'High',    'Scheduled', 7,'Replace suction and discharge valves and seats.'),
  ('PM-007','AST-003','Derrick Structural Inspection',  'Inspection',        365,'2024-02-01','2025-02-01','SGS Inspection',        12,6000, 'Critical','Scheduled',30,'Full visual and NDT inspection of derrick per API 4F.'),
  ('PM-008','AST-005','Generator Oil & Filter Change',  'Oil Change',        250,'2024-10-01','2025-06-08','CAT Dealer Team',        6,2800, 'Normal',  'Scheduled',14,'250-hour PM service. Oil, fuel filters, air filters, coolant check.'),
  ('PM-009','AST-005','Generator Load Bank Test',       'Electrical Check',   90,'2024-11-15','2025-02-13','Hamdan Ali',             4,1500, 'Normal',  'Scheduled',14,'Full load bank test at 100% rated capacity for 4 hours.'),
  ('PM-010','AST-008','H2S Sensor Calibration',         'Calibration',        30,'2024-12-28','2025-01-27','Safety Officer',         2,400,  'Critical','Scheduled', 7,'Calibrate all H2S sensors with certified calibration gas.'),
  ('PM-011','AST-008','SCBA Equipment Inspection',      'Safety Check',      180,'2024-07-01','2025-01-01','Safety Officer',         3,600,  'Critical','Scheduled',14,'Inspect all 30 SCBA units. Check cylinder pressure, regulator function, face seal.'),
  ('PM-012','AST-006','VSAT Antenna Alignment',         'General Service',   180,'2024-06-15','2024-12-15','Hughes Network Tech',    3,1800, 'Normal',  'Completed',14,'Check and realign satellite dish. Clean feed horn. Test link quality.'),
  ('PM-013','AST-015','Jar Jar Tool Function Test',     'Inspection',         90,'2024-10-20','2025-01-18','Fishing Tool Engr',      4,2000, 'High',    'Scheduled', 7,'Function test hydraulic jars at rated load.'),
  ('PM-014','AST-003','Triplex Pump Packing Change',    'General Service',    45,'2024-11-10','2024-12-25','Ahmed Khalid',           6,1800, 'High',    'Completed', 7,'Replace all piston packing sets on 3 pumps.')
ON CONFLICT (id) DO NOTHING;

-- ─── MAINTENANCE LOGS ────────────────────────────────────────────────────────
INSERT INTO maintenance_logs (schedule_id,completion_date,performed_by,hours,cost,parts_used,notes) VALUES
  ('PM-012','2024-12-14','Hughes Network Tech', 2.5,1700,'None',                          'Realigned dish 0.3° east. Signal improved from 74% to 91%.'),
  ('PM-014','2024-12-24','Ahmed Khalid',         7,  2100,'Garlock packing kits x18',      'All 3 pumps serviced. Found hairline crack on pump #2 liner — replaced.')
ON CONFLICT DO NOTHING;

-- ─── TRANSFERS ───────────────────────────────────────────────────────────────
INSERT INTO transfers (id,asset_id,asset_name,current_loc,destination,dest_rig,dest_company,priority,type,requested_by,request_date,required_date,reason,instructions,status,ops_approved_by,ops_approved_date,ops_action,ops_comment) VALUES
  ('TR-001','AST-001','Top Drive System','Ghawar Field','NABORS 1250-E – Permian Basin','NABORS 1250-E','NABORS Industries','High','Rig to Rig','Sara Al-Rashid','2025-01-10','2025-01-25',
   'Top drive required for well P-44 spud at Permian Basin. Current rig shifting to standby.',
   'Handle with crane only. Do not tilt >15°.','Ops Approved',
   'James Miller','2025-01-11','approve','Approved. Coordinate with logistics team for transport scheduling.'),
  ('TR-002','AST-003','Mud Pumps (3x)','Permian Basin','Workshop – Bay 4 (Overhaul)',NULL,NULL,'Critical','For Maintenance','Ahmad Mohammed','2025-01-08','2025-01-12',
   'Mud pumps require emergency overhaul. Liner wear detected on all 3 units.',
   'Transport on flatbed. Keep units upright. Drain fluid before transport.','Completed',
   'James Miller','2025-01-08','approve','Critical – approved immediately.',
   'Sara Al-Rashid','2025-01-09','approve'),
  ('TR-003','AST-007','H2S Detection System','Permian Basin','Khafji Field – KJO Land Rig #3','KJO Land Rig #3','Al-Khafji Joint Operations','Normal','Field to Field','David Chen','2025-01-15','2025-01-30',
   'H2S detection system requested by KJO site for new drilling area with known H2S presence.',
   'Calibrate before transport. Include all sensor heads and control unit.','Pending',NULL,NULL,NULL,NULL)
ON CONFLICT (id) DO NOTHING;

-- Fix TR-002 mgr approval (separate update since INSERT doesn't handle it cleanly inline)
UPDATE transfers SET mgr_approved_by='Sara Al-Rashid', mgr_approved_date='2025-01-09', mgr_action='approve', mgr_comment='Approved. Schedule overhaul with maintenance team ASAP. Budget pre-approved.' WHERE id='TR-002';

-- ─── USERS ───────────────────────────────────────────────────────────────────
INSERT INTO app_users (name, role, dept, email, color, initials) VALUES
  ('Ahmad Mohammed', 'Admin',         'Asset Management', 'a.mohammed@company.com', '#0070F2', 'AM'),
  ('Sara Al-Rashid', 'Asset Manager', 'Operations',       's.alrashid@company.com', '#8B5CF6', 'SR'),
  ('James Miller',   'Viewer',        'Finance',          'j.miller@company.com',   '#107E3E', 'JM'),
  ('Layla Hassan',   'Editor',        'Contracts',        'l.hassan@company.com',   '#E9730C', 'LH'),
  ('David Chen',     'Viewer',        'Engineering',      'd.chen@company.com',     '#BB0000', 'DC'),
  ('Fatima Al-Zahra','Editor',        'Maintenance',      'f.alzahra@company.com',  '#0070F2', 'FZ')
...
INSERT INTO notifications (icon,kind,title,description,time_label,is_read) VALUES
  ...
  ('fas fa-tools',               'warning','Maintenance Alert',       'Asset AST-003 (BOP Unit) requires scheduled maintenance',  '4 hours ago',false),
  ('fas fa-file-import',         'info',   'Import Completed',        '12 assets imported successfully from Excel file',           'Yesterday',  false),
  ('fas fa-check-circle',        'success','Asset Activated',         'AST-009 (Top Drive System) has been activated',             '2 days ago', false),
